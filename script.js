document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- REFACTORED Mobile Hamburger Menu Logic ---
    hamburgerMenu.addEventListener('click', (e) => {
        ...
        // This is the key fix:
        // When opening the menu, always ensure the EXPLORER view is active.
        if (isOpening) {
            document.querySelectorAll('.sidebar').forEach(view => {
                view.style.display = view.id === 'explorer-view' ? 'flex' : 'none';
            });
            // Also update the active icon in the activity bar
            activityBarIcons.forEach(i => i.classList.remove('active'));
            document.querySelector('.action-icon[data-view="explorer"]').classList.add('active');
        }
    });
...
There's something incredibly thrilling about taking those first steps in brand development: crafting the initial concept, assembling the right team, and orchestrating different personalities to work toward a shared vision.
...
Today, I dedicate most of my time to my work at **WATANAI.COM** as CEO and Co-founder, where I handle product ownership, R&D, and content strategy. For the past two years, AI and I have been inseparable – it has transformed my previously monochrome world into something far more vibrant and engaging.
## Interests & Future Plans
I'm deeply passionate about podcasting and have considered launching one outside the tech sphere. Like most tech enthusiasts since 2021, my focus has been laser-sharp on AI-related topics – and that passion continues to drive everything I do.
`,
        'skills.py': `
class SajjadSkills:
    def __init__(self):
        self.languages = ["Python", "JavaScript", "HTML", "CSS"]
        self.frameworks = { "backend": ["Flask", "Django", "FastAPI"], "frontend": ["React", "Vue.js"] }
...
`
    ...
    // The placeholder functions for brevity:
    ...
    function createEditorContent(filename) { const contentDiv = document.createElement('div'); contentDiv.className = 'editor-content'; contentDiv.dataset.file = filename; contentDiv.innerHTML = `<pre>${fileContents[filename] || ''}</pre>`; editorContainer.appendChild(contentDiv); openFiles[filename] = { tab: null, content: contentDiv }; if (filename === 'contact.html') { const form = contentDiv.querySelector('#contactForm'); if(form) form.addEventListener('submit', handleFormSubmit); } }
    ...
    const terminal = document.getElementById('terminal'); const terminalInput = document.getElementById('terminal-input'); const terminalBody = document.getElementById('terminal-body'); const closeTerminalBtn = document.getElementById('close-terminal'); const toggleTerminalBtn = document.getElementById('toggle-terminal'); const terminalHeader = document.querySelector('.terminal-header');
    ...
    if(terminalHeader) terminalHeader.addEventListener('click', (e) => { if(e.target === terminalHeader || e.target.tagName === 'SPAN') { toggleTerminal(); } });
    function toggleTerminal() { terminal.classList.toggle('hidden'); const icon = toggleTerminalBtn.classList; icon.toggle('fa-chevron-up'); icon.toggle('fa-chevron-down'); }
